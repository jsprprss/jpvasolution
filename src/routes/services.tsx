    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24">
        ...
        <div className="flex flex-wrap gap-2">
          {tools.map((t) => (
            <span key={t} className="px-4 py-2 border border-border rounded-md font-mono text-xs tracking-wider bg-card">
              {t}
            </span>
          ))}
        </div>
        <div className="mt-12">
          <Link to="/contact" className="inline-flex items-center gap-3 px-6 py-3 rounded-md bg-primary text-primary-foreground font-mono text-xs uppercase tracking-widest glow hover:opacity-90 transition">
            Scope a project →
          </Link>
        </div>
      </section>
